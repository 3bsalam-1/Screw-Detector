...
from screw_detector.config import SAHIConfig  # noqa: E402
from screw_detector.inference import BaselineInference, SAHIInference  # noqa: E402
from screw_detector.utils import (  # noqa: E402
    calculate_metrics,
...
def parse_args():
    ...
    # Data arguments
    parser.add_argument(
        "--data", type=str, default="data/configs/data.yaml", help="Path to data configuration file"
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument("--imgsz", type=int, default=640, help="Image size for baseline inference")

    # SAHI arguments
    parser.add_argument("--slice-size", type=int, default=640, help="Slice size for SAHI")
    parser.add_argument("--overlap", type=int, default=128, help="Overlap between slices in pixels")
    parser.add_argument("--confidence", type=float, default=0.6, help="Confidence threshold")
    parser.add_argument("--nms-threshold", type=float, default=0.6, help="NMS threshold for SAHI")
    ...
    parser.add_argument("--save-table", action="store_true", help="Save results table as CSV")
...
def evaluate_baseline(model_path: str, test_images: list[Path], imgsz: int, device: str) -> tuple:
...
def main():
    ...
    if args.strategy == "both":
        ...
        for size_bin in baseline_size_recall.keys():
            ...
            improvement = (
                (sahi_recall - baseline_recall) / baseline_recall * 100
    ...
    if args.save_plots:
        ...
        if args.strategy == "both":
            ...
            plot_size_based_recall(
                size_recall_comparison, output_dir / "size_recall_comparison.png"
            )
