...
from screw_detector.utils import visualize_comparison, visualize_predictions  # noqa: E402
...
def parse_args():
    ...
    parser.add_argument("--show", action="store_true", default=True, help="Show output in a window")
    parser.add_argument(
        "--no-show", action="store_false", dest="show", help="Do not show output in a window"
    )
...
def run_image_demo(args):
    ...
    if args.strategy == "baseline":
        baseline = BaselineInference(
            args.model, confidence_threshold=args.confidence, device=args.device
        )
