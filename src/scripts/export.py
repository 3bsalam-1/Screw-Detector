...
from screw_detector.config import ExportConfig  # noqa: E402
from screw_detector.models import ModelExporter, get_model_size  # noqa: E402
...
def parse_args():
    ...
    parser.add_argument(
        "--no-simplify", action="store_false", dest="simplify", help="Do not simplify ONNX model"
    )
    parser.add_argument("--dynamic", action="store_true", help="Use dynamic axes for ONNX export")

    # OpenVINO arguments
    parser.add_argument(
        "--int8", action="store_true", help="Export in INT8 (quantized) for OpenVINO"
    )
...
def main():
    ...
    if args.format == "all":
        ...
        export_path = exporter.export_onnx(
            half=args.half, simplify=args.simplify, dynamic=args.dynamic
        )
        ...
        export_path = exporter.export_openvino(half=args.half, int8=args.int8)
