...
def visualize_predictions(
    ...
    for det in detections:
        ...
        cv2.putText(
            img, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), 1
        )
...
def visualize_comparison(
    ...
    # Add labels
    cv2.putText(comparison, "Baseline", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(comparison, "SAHI", (w + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def calculate_metrics(
    predictions: list[list[dict]], ground_truth: list[list[dict]], iou_threshold: float = 0.5
) -> dict[str, float]:
    """
...
def calculate_size_based_recall(
    ...
    for preds, gt, (_img_w, _img_h) in zip(predictions, ground_truth, image_sizes):
...
def plot_metrics_comparison(
    metrics_dict: dict[str, dict[str, float]], output_path: Optional[Union[str, Path]] = None
) -> None:
    """
...
def create_results_table(
    results_dict: dict[str, dict[str, float]], output_path: Optional[Union[str, Path]] = None
) -> pd.DataFrame:
    """
...
def print_results_table(results_dict: dict[str, dict[str, float]], title: str = "RESULTS") -> None:
    """
...
def get_image_size(image_path: Union[str, Path]) -> tuple[int, int]:
    ...
    try:
        ...
        raise ValueError(f"Could not read image: {image_path}") from None
...
def load_ground_truth(
    label_path: Union[str, Path], image_width: int, image_height: int
) -> list[dict]:
    """
