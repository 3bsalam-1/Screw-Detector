...
class YOLOModel:
    ...
    @classmethod
    def from_pretrained(cls, model_name: str = "yolov8s.pt", device: str = "cpu") -> "YOLOModel":
        """
...
class ModelTrainer:
    ...
    def __init__(self, model_name: str = "yolov8s.pt", config: Optional[TrainingConfig] = None):
        """
    ...
    def train(self, **kwargs) -> dict:
        ...
        return {"results": results, "training_time": training_time, "config": train_params}
    ...
    def tune(
        ...
        return {"results": results, "tuning_time": tuning_time}
...
class ModelExporter:
    ...
    def __init__(self, model_path: Union[str, Path], config: Optional[ExportConfig] = None):
        """
    ...
    def export_onnx(
        self, half: bool = False, simplify: bool = True, dynamic: bool = False, **kwargs
    ) -> Path:
        """
        ...
        export_path = self.model.export(
            format="onnx", half=half, simplify=simplify, dynamic=dynamic, **kwargs
        )

        print(f"\nExported to: {export_path}")
        return Path(export_path)

    def export_openvino(self, half: bool = False, int8: bool = False, **kwargs) -> Path:
        """
        ...
        export_path = self.model.export(format="openvino", half=half, int8=int8, **kwargs)

        print(f"\nExported to: {export_path}")
        return Path(export_path)

    def export_torchscript(self, **kwargs) -> Path:
        """
        ...
        export_path = self.model.export(format="torchscript", **kwargs)

        print(f"\nExported to: {export_path}")
        return Path(export_path)

    def export_all(self, output_dir: Optional[Union[str, Path]] = None) -> dict[str, Path]:
        """
        ...
        exports["onnx"] = self.export_onnx(
            half=self.config.half, simplify=self.config.simplify, dynamic=self.config.dynamic
        )

        # Export to OpenVINO
        exports["openvino"] = self.export_openvino(half=self.config.half, int8=self.config.int8)
